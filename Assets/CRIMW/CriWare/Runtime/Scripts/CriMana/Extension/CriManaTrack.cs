		...
		public override Playable CreateTrackMixer(PlayableGraph graph, GameObject go, int inputCount) {
			...
			if (director != null) {
				...
				foreach (var clip in bh.m_clips) {
					var clipAsset = (clip.asset as CriManaClip);
					clipAsset.m_clip = clip;
					if (clipAsset.m_movieData != null) {
						clip.displayName = clipAsset.m_movieData.name;
					} else if (string.IsNullOrEmpty(clipAsset.m_moviePath) == false) {
						clip.displayName = System.IO.Path.GetFileNameWithoutExtension(clipAsset.m_moviePath);
					}
				}
