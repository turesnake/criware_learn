...
namespace CriWare
{
    namespace CriTimeline.Mana
    {
        ...
        public class CriManaMixerBehaviour : PlayableBehaviour
        {
            ...
            private bool PlayMovie(CriManaClip clipAsset, int startFrame, double startTime)
            {
                ...
                if (IsEditMode)
                {
                    if (m_boundMovieMaterial is CriManaMovieController)
                    {
                        ...
                        {
                            Debug.LogWarning(
                                "[CRIWARE] Missing render target on CriManaMovieController: Please add a renderer to the GameObject or specify the target manually.");
                            return false;
                        }
                    }
                    else if (m_boundMovieMaterial is CriManaMovieControllerForUI)
                    {
                        ...
                        {
                            Debug.LogWarning(
                                "[CRIWARE] Missing render target on CriManaMovieControllerForUI: Please add a renderer to the GameObject or specify the target manually.");
                            return false;
                        }
                    }
                }
                ...
                if (clipAsset != null)
                {
					...
					Debug.Log("Set <color=white>new</color> movie");
                        ...
                        if (clipAsset.m_movieData != null)
                        {
                            ...
                            m_boundMovieMaterial.player.SetData(clipAsset.gcHandle.AddrOfPinnedObject(),
                                clipAsset.m_movieData.bytes.LongLength);
                        }
                        else if (string.IsNullOrEmpty(clipAsset.m_moviePath) == false)
                        {
...
#endif
                m_boundMovieMaterial.timerType =
                    m_frameSync ? CriMana.Player.TimerType.User : CriMana.Player.TimerType.Audio;
			...
			Debug.Log("<color=green>PrepareMovie</color>");
                ...
                {
                    m_boundMovieMaterial.player.SetFile(null, GetManaPath(clipAsset.m_moviePath));
                }
                else
                {
                    /* empty clip */
            ...
            {
                var vendorPath = Path.Combine(Application.persistentDataPath, "Product");
                return vendorPath;
            }

            private string BuildFilePath(string assetPath)
            {
                var installPath = GetInstallPath();
                var assetFolder = "CriWare";

                var path = Path.Combine(installPath, assetFolder);
            ...
            private string GetManaPath(string moviePath)
            {
                var documentPath = BuildFilePath(moviePath);
                if (!File.Exists(documentPath))
                {
                    documentPath = Path.Combine(CriWare.Common.streamingAssetsPath, moviePath);
                }
            ...
            private async void ProcessFrameOnSeeking(TimelineClip activeClip, CriManaClip clip, double frameTime)
            {
                if (m_boundMovieMaterial == null ||
                    m_boundMovieMaterial.player == null ||
                    activeClip == null ||
                ...
                if (clip.m_movieData != null)
                {
                    ...
                    {
                        m_boundMovieMaterial.player.SetData(clip.gcHandle.AddrOfPinnedObject(),
                            clip.m_movieData.bytes.LongLength);
                    }
                    else
                    {
                        /* empty clip */
                ...
                else if (string.IsNullOrEmpty(clip.m_moviePath) == false)
                {
                    m_boundMovieMaterial.player.SetFile(null, GetManaPath(clip.m_moviePath));
                }
                else
                {
                    return;
                ...
                {
                    await Task.Delay(1);
                    if (m_boundMovieMaterial.player == null)
                    {
                        return;
                    } /* clip been moved out of track */

                    if (m_currentSeekingFrameTime != frameTime)
                    {
                        break;
                    } /* seeking frame changed */
            ...
            public override void ProcessFrame(Playable playable, FrameData info, object playerData)
            {
                ...
                switch (m_movieMixerState)
                {
                    case MovieMixerState.Stopped:
                        ...
                        {
                            if (this.PlayMovie(clipAsset,
                                    clipAsset.GetSeekFrame(frameTime - activeClip.start, clipAsset.m_loopWithinClip),
                                    frameTime))
                            {
                    ...
                    case MovieMixerState.Ready:
                        if ((clipState == ClipState.Play || clipState == ClipState.Seek) &&
                            clipAsset.guid == m_lastClipId)
                        {
                ...
                if (m_boundMovieMaterial != null && m_boundMovieMaterial.player != null)
                {
                    ...
                    do
                    {
                        /* Forcing update on intermediate states */
                        if (startTime >= 0)
                        {
                            m_boundMovieMaterial.PlayerManualUpdate();
                        }
                ...
                var weight = Mathf.Min(
                    clipAsset.m_fadeinCurve.Evaluate(Mathf.Approximately(clipAsset.m_fadeinDuration, 0f)
                        ? 1f
                        : Mathf.InverseLerp((float) activeClip.start,
                            (float) activeClip.start + clipAsset.m_fadeinDuration, (float) frameTime)),
                    clipAsset.m_fadeoutCurve.Evaluate(Mathf.Approximately(clipAsset.m_fadeoutDuration, 0f)
                        ? 0f
                        : Mathf.InverseLerp((float) activeClip.end - clipAsset.m_fadeoutDuration,
                            (float) activeClip.end, (float) frameTime))
                );
			...
			Debug.Log("<color=white>OnGraphStart" + (IsEditMode ? " (Editor)</color>" : "</color>"));
                ...
                foreach (var clip in m_clips)
                {
                    ...
                    if (manaClip.m_movieData == null)
                    {
                        continue;
                    }

                    if (m_gcHandleList.ContainsKey(manaClip.m_movieData.GetInstanceID()) == false)
                    {
                        manaClip.gcHandle = GCHandle.Alloc(manaClip.m_movieData.bytes, GCHandleType.Pinned);
                        m_gcHandleList.Add(manaClip.m_movieData.GetInstanceID(), manaClip.gcHandle);
                    }
                    else
                    {
                        manaClip.gcHandle = m_gcHandleList[manaClip.m_movieData.GetInstanceID()];
                    }
                }
            }
                ...
                if (IsEditMode)
                {
                    if (CriManaPlugin.IsLibraryInitialized() == false)
                    {
                        ...
                        else
                        {
                            ...
                            Debug.Log(
                                "[CRIWARE] Timeline / Mana: Can't find CriWareInitializer component; Using default parameters in edit mode.");
                        }
                    }
                }
