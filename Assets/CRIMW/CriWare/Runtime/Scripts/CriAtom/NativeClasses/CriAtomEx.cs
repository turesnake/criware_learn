...
/**
	...
	---
	存储了一堆 Atom library 的 settings 函数, 和 变量类型; 可以看见它们在整个插件库中被使用;
...
public static class CriAtomEx
{
	 ...
	 * Sound renderer type
		...
		Represents the type of sound renderer that ::CriAtomExPlayer creates internally.
	 */
	 public enum SoundRendererType {
		Default = 0,
		Native = 1,
		Asr = 2,   // Atom Sound Renderer
		Hw1 = 1,
		Hw2 = 9,
	}
	...
	/**
		...
		A data type for specifying the behavior when ::CriAtomExPlayer allocates a Voice.
	...
	/**
		Biquad Filter type -- 二阶滤波类型
	 
		A data type that specifies the type of Biquad Filter.

	 * Used in the ::CriAtomExPlayer::SetBiquadFilterParameters .
	...
	/**
		...
		---
		指定 unpause(取消暂停) 的对象 是哪些;

	 * Used as an argument to the ::CriAtomExPlayer::Resume() and ::CriAtomExPlayback::Resume() functions.

	 * <seealso cref='CriAtomExPlayer::Resume'/>
	 * <seealso cref='CriAtomExPlayback::Resume'/>
	 */
	public enum ResumeMode {
		AllPlayback = 0,            // Resumes playback regardless of how it was paused -- 不关心它是怎么被暂停的, 直接重放;
		PausedPlayback = 1,         // Resumes playback only for the sound paused by the Pause function -- 只重放那些被 Pause 函数暂停的 声音;
		PreparedPlayback = 2,       // Starts playback of the sound "prepared for playback using the Prepare function" -- 重放那些: 使用 Prepare 函数整备的 声音;
	}


	/**
		Panning type
		panning: 声像, 就是控制 左右声道用的

	 * A data type for specifying how to perform the localization calculation.
	 * Used in the ::CriAtomExPlayer::SetPanType .
	 ...
	 * Used in the ::CriAtomExPlayer::SetVoiceControlMethod .<br/></para>

	 * <seealso cref='CriAtomExPlayer::SetVoiceControlMethod'/>
	 */
	public enum VoiceControlMethod {
		PreferLast = 0,             /**< Last-note priority   -- 排后面的 声音 的优先级更高 --*/
		PreferFirst,                /**< First-note priority  -- 排前面的 声音 的优先级更高 --*/
	}
	 ...
	 * Used in functions such as ::CriAtomExPlayer::GetParameterFloat32 .
	...
	public enum Parameter {
		...
		BandPassFilterCofLow    = 17,   /**< Low cutoff frequency of the BandPass Filter  -- 带通滤波器的 低截止频率 */
		BandPassFilterCofHigh   = 18,   /**< High cutoff frequency of the BandPass Filter -- 带通滤波器的 高截止频率 */
		BiquadFilterType        = 19,   /**< Filter type of the Biquad Filter 	-- 双二阶滤波器 的滤波类型 */
		BiquadFilterFreq        = 20,   /**< Biquad Filter frequency 			-- 双二阶滤波器 频率 */
		BiquadFIlterQ           = 21,   /**< Q value of the Biquad Filter 		-- 双二阶滤波器 Q 值 */
	...
	/**
		Speaker ID
		An ID for specifying the speaker that outputs sound.  扬声器 id

	 * Used in the ::CriAtomExPlayer::SetSendLevel .</para>
	...
	/**
	 * <summary>Format type</summary>  -- 声音格式

		A data type used to specify the "sound format" to be played by the AtomExPlayer.

	 * Used in the ::CriAtomExPlayer::SetFormat .<br/></para>
	...
	/**
		Output speaker angle (5.1ch)

	 * Sets the angle (arrangement) of the output speakers used when calculating Pan3D or 3D Positioning for virtual speakers.
	 ...
	 * by calling the ::CriAtomEx::ControlVirtualSpeakerSetting function. <br/></para>
	 * </remarks>
	 */
	...
	/**
		...
		参数: sw -- Switch (False = disable, True = enable) 默认参数为 disabled
	 ...
	 * set by the ::CriAtomEx::SetVirtualSpeakerAngle function.
	 ...
	 * The change will be applied from the playback of the next Voice.
	 */
	...
	/**
		A structure for getting the AISAC control information;

	 * A structure for getting the AISAC control information.
	 * Passed to the ::CriAtomExAcb::GetUsableAisacControl function as an argument.
	...
	public struct AisacControlInfo 
	{
		...
		public uint             id;         /**< AISAC control ID */
	...
	public struct CuePos3dInfo
	{
		...
		public ushort distanceAisacControl;             /**< Distance attenuation AISAC control */
		public ushort listenerBaseAngleAisacControl;    /**< Listener reference angle AISAC control */
		public ushort sourceBaseAngleAisacControl;      /**< Sound source reference angle AISAC control */
		public ushort listenerBaseElevationAisacControl;/**< Listener reference elevation AISAC control */
		public ushort sourceBaseElevationAisacControl;  /**< Sound source reference elevation AISAC control */
	...
	/**
	Structure for acquiring(获得) game variable information;

	This is the structure used to get information on a game variable.

	 * Pass it to the ::CriAtomEx::GetGameVariableInfo function as an argument. <br/></para>
	...
	public enum CueType
	{
		Polyphonic,             /**< Polyphonic -- 多声部 */
		Sequential,             /**< Sequential -- 序列的 */
		Shuffle,                /**< Shuffle playback -- 乱序播放 (洗牌) */
		Random,                 /**< Random */
		RandomNoRepeat,         /**< Random discontinued (randomly play sounds other than the one played previously) */
		SwitchGameVariable,     /**< Switch playback (switches tracks to be played by referencing the game variable) -- 通过引用游戏变量切换要播放的曲目 */
		ComboSequential,        /**< Combo Sequential 	(Sequential when consecutive combo is successful within the "combo time", 
														and returns to "combo loopback" point when the end is reached) 
									组合顺序;
								*/
	 ...
	 * <summary>Cue information</summary>
		...
		Detailed information of the Cue.
	...
	/**
	 * A structure for getting the AISAC information
	 ...
	 * Used in the ::CriAtomEx::GetPerformanceInfo .</para>
	 * </remarks>
	...
	A structure indicating the usage status of various resources.
	...
	*/
	 ...
	 * Attaching the DSP bus settings
	 	...
	 	参数: settingName: Name of the DSP bus setting
		...
		使用示范:
	 	...
	 	so call this function at a timing when load fluctuations(波动) is accepted such as when switching scenes.
		---
		调用此函数会暂时阻塞 Atom lib 的服务器处理。
		若在 声音回放 阶段调用此函数, 可能发生类似 声音中断 的问题;
		所以最好在 场景切换 时调用本函数;
	...
	/**
		...
		Calling this function blocks the server processing of the Atom library for a while.
		If this function is called during sound playback, problems such as sound interruption may occur,
		so call this function at a timing when load fluctuations is accepted such as when switching scenes.
		---
		调用此函数会暂时阻塞 Atom lib 的服务器处理。
		若在 声音回放 阶段调用此函数, 可能发生类似 声音中断 的问题;
		所以最好在 场景切换 时调用本函数;
	 ...
	 * If you want reproducibility in each AtomExPlayer, use the ::CriAtomExPlayer::SetRandomSeed function.
	 * <br/>
	 * If you do not need reproducibility and want to change the random number seed for each execution,
	 * use the ::CriAtomConfig::useRandomSeedWithTime property instead of this function.
	 * <br/></para>
	 * <para header='Note'>This function must be called before generating ::CriAtomSource or ::CriAtomExPlayer .
	...
	/**
	Prevents interruption to the server process; -- 防止服务器进程中断；

	Prevents the lock of interruption to server processing.

	 * The interruption to the server processing is prevented after this function is called until the ::CriAtomEx::Unlock function is called.
	 * If you want to reliably call multiple APIs in the same audio frame, 
	 	use this function to prevent interruption to the server processing and then call those functions.
	 	---
		在本函数被调用后, 服务器进程的 中断 就被阻止了, 直到调用 ::CriAtomEx::Unlock() 后才会恢复;
		如果你想在 audio地同一帧 可靠地调用多个 apis, 请先调用本函数来 阻止中断, 然后再调用;
	 ...
	 * <para header='Description'>Removes the lock for preventing interruption of server processing by the ::CriAtomEx::Lock function.</para>
	...
	Whether to enable the restoration process;
...
/**
	A class for controlling parameters for each Category.(类别)
...
public static class CriAtomExCategory
{
	...
	{
		Ducker = 0,                     // Ducker, 由 音频引擎去自动控制避让处理 的一个 避让器;  (一般选这个)
		AisacModulationTrigger,         // AISAC modulation trigger (调制触发器): 选此项, 则由程序去控制避让
	}
	...
	{
		Volume = 0,                     // Volume ducker: 						-- 音量 的 避让处理
		AisacControlValue,              // Ducker of the AISAC control value:   -- aisac控制值 的 避让处理
	}


	/**
		Ducking Curve Type by REACT
		避让处理的 曲线类型;
	...
	{
		Linear = 0,                     // straight line  -- 直线
		Square,                         // Slow transition -- 缓慢过度
		SquareReverse,                  // high speed change -- 高速改变
		SCurve,                         // S curve  --
		FlatAtHalf,                     // Inverted S curve -- 倒 s 曲线
	}


	/**
		REACT fade parameter structure

		A structure for getting/setting the fade drive parameter information of REACT.
		---
		"驱动 避让衰减的 淡入淡出 的 参数信息" 的结构体;
	...
	public struct ReactFadeParameter
	{
		...
		public float                    curveStrength;  // Curve strength (0.0f - 2.0f) 猜测是 曲线强度 
		public System.UInt16            fadeTimeMs;     // Fading time (milliseconds) 衰减的时长
	}


	/**
		REACT hold type (for maintaining the decay time 用于保持衰减时间). 
	...
	{
		WhilePlaying,                   // Do holding while playing -- 在 playing 时一直 hold
		FixedTime,                      // Hold for a fixed time    -- hold 一段固定的时长
	}
	...
	/**
		...
		A structure for getting/setting the ducker driving parameter information by REACT.
	...
	public struct ReactDuckerParameter
	{
		...
		{
			public System.UInt16 id;        /**< AISAC control ID */
	 ...
	 * The specification is the same as the ::CriAtomExCategory::Pause(int, bool) function, except that
	 ...
	 * the AtomExPlayer or playback sound (pause with ::CriAtomExPlayer::Pause or ::CriAtomExPlayback::Pause function),
	 ...
	 * Sets the AISAC control value for the Category by specifying the name.
	 * The specification is the same as the ::CriAtomExCategory::SetAisacControl function, except that
	 * the Category and AISAC control are specified by the name.
	...
	*/
	 ...
	 * For the AISACs attached to the Category, only the AISAC control value set to the Category is always referenced.
	 	---
		对于那些设置为 Cue 或 Track 的 aisac 值时, 设置给 Category 的 aisac 控制值, 优先级要高于设置给 player 的 aisac 控制值;
		对于那些 附加到 Category 的 AISAC，始终仅引用 设置为该 Category 的 AISAC 控制值。
...
/**
	...
	---
	使用由 "CRI Atom Craft" 创建的 序列数据 的 class;
...
public static class CriAtomExSequencer
{
	...
	/**
	 	Structure to get information about a sequence event;
		This is the structure used to get information about a sequence event.
	...
	/**
		...
		参数: eventParamsString -- Event parameter string;
	 ...
	 * Each piece of information is concatenated(串联的) and passed as one string with the specified delimiter character in between.
	 * Parse the required parameters from the string before use.
	 ---
	 上述每段信息, 都被串联起来组成一个 字符串, 中间使用特定的分隔符;
	 使用前从这个字符串 中解析所需的参数。
	...
	参数: criAtomExSequenceInfo -- Event information structure
	 ...
	 * The registered callback function will be executed when the application main thread is updated immediately after processing the callback event.
	 	---
		注册一个回调函数，由 "同步化嵌套在 cue 中" 的 callback marker 执行。
		在 处理回调事件后 立即执行 app 的 主线程 update(0 时，将执行这些 登记的 回调函数。
	 ...
	 * \deprecated 已废弃;
	 * 不建议删除的API。
	 * CriAtomExSequencer.OnCallback event 中所述修改相应参数的值。
...
/**
A class for using "beat synchronization" data.   拍同步 数据;
...
public static class CriAtomExBeatSync
{
	...
	public struct Info {
		...
		public uint     barCount;           /**< The number of Measures 测量的数量 */
		public uint     beatCount;          /**< The current count of beats */
		public float    beatProgress;       /**< Beat progress (0.0f to 1.0f) */
		public float    bpm;                /**< Tempo (beats/minute) 每分钟多少节拍 */
		public int      offset;             /**< Sync offset (ms) */
		public uint     numBeats;           /**< The number of beats */
	}

	/**
	Beat synchronization callback  节拍同步 回调;

	Beat synchronization position detection information

	"CbFunc": 就是 callback func;
...
public static class CriAtomExLatencyEstimator
{
	 ...
	 * It can get using the ::CriAtomExLatencyEstimator::GetCurrentInfo function.<br/>
	 * <br/></para>
	 * </remarks>
	 */
	public enum Status {
