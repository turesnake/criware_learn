...
#error unsupported platform
	...
	{
		UsedVoicesInfo info;
		CRIWARE059D1893((int)voicePoolId, out info.numUsedVoices, out info.numPoolVoices);
		return info;
	}
	...
	private static extern void CRIWARE059D1893(int voice_pool_id, out int num_used_voices, out int num_pool_voices);
	...
	#endif
	#else
	private static void CRIWARE059D1893(int voice_pool_id, out int num_used_voices, out int num_pool_voices)
...
public class CriAtomExRawPcmVoicePool: CriAtomExVoicePool
{
	 ...
	 * Even if the number of channels or and sampling rate is specified for ::CriAtomExPlayer ,
